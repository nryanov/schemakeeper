...
object Converters {
  def schemaInfoToSchemaMetadata(schemaInfo: SchemaInfo): SchemaMetadata =
    SchemaMetadata.instance(
      ...
      schemaInfo.schemaHash,
      SchemaType.findByName(schemaInfo.schemaTypeName)
    )

  def subjectInfoToSubjectMetadata(subject: Subject): SubjectMetadata =
    SubjectMetadata.instance(
      subject.subjectName,
      CompatibilityType.findByName(subject.compatibilityTypeName),
      subject.isLocked
    )
}
