...
final case class SchemaIdDoesNotExist(schemaId: Int) extends SchemaKeeperError(s"Schema with id: $schemaId does not exist")
...
final case class SubjectIsNotConnectedToSchema(subject: String, schemaId: Int) extends SchemaKeeperError(s"Subject: $subject is not connected to schema: $schemaId")
